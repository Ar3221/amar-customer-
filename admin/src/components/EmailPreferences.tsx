...
export function EmailPreferences({ currentUser }: EmailPreferencesProps) {
  ...
  const items: { key: keyof EmailPreferencesState; label: string; desc: string }[] = [
    ...
    { key: 'supervisor_alerts', label: 'New Order Alerts', desc: 'Incoming customer order requests (supervisors)' },
    { key: 'escalation_alerts', label: 'Escalation Alerts', desc: 'SLA breach and critical pending orders (admins)' },
    { key: 'inventory_alerts', label: 'Inventory Warnings', desc: 'Low stock and reorder threshold alerts' },
    { key: 'high_value_alerts', label: 'High Value Orders', desc: 'Orders exceeding the executive review threshold' },
  ...
  return (
    <div className="glass-panel p-6 rounded-xl space-y-5 bg-slate-900/30 border border-industrial-800">
      <div className="flex items-center gap-3 border-b border-industrial-800 pb-4">
        ...
        <div>
          <h4 className="font-bold text-sm uppercase text-slate-200 tracking-wider">Email Communication Preferences</h4>
          <p className="text-[10px] text-slate-500 font-mono-custom mt-0.5">Manage workflow notification delivery</p>
        </div>
      </div>
      ...
      <div className="space-y-2 opacity-90">
        {items.map(({ key, label, desc }) => (
          <label
            key={key}
            className="flex items-start justify-between gap-4 p-3 rounded-lg bg-slate-900/50 border border-industrial-800/60 cursor-pointer hover:border-slate-700 transition"
          >
            <div>
